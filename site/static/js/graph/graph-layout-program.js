...
;(function () {
  ...
  function digraphValueData(a, b) {
    ...
    return extend({ angle: Vector.angle(ab) }, Vector.midPoint(a, b))
  }
  ...
  function createPath(d, meta, margin) {
    ...
    if (u.index === v.index) {
      ...
      extend(current, {
        ...
        unitInverse: Vector.orthogonal(Vector.unit(Vector.sub(v, u)))
      })

      mid = Vector.add(
        current.mid,
        Vector.scale(current.unitInverse, Math.floor(current.count / 2) * margin * current.direction)
      )
  ...
  var graphLayout = function (options) {
    ...
    var svg = d3.select(options.target).selectAll('svg.graph').data([options])
    ...
    if (svgEnter.size()) {
      ...
      svg
        .node()
        .layout.linkDistance(function (d) {
        ...
        .groups(data.groups)
      if (data.tree) {
        layout.flowLayout('y', 50).symmetricDiffLinkLengths(20)
      }
    }
    ...
    var groups = svg.selectAll('.group').data(data.groups)
    groups
      .enter()
      .append('rect')
      .attr('rx', 8)
    ...
    // edges
    var links = svg.selectAll('g.link').data(data.links, function (d) {
      return d.id
    })
    ...
    function incoming(d, over) {
      if (data.highlightIncomingEdges) {
        svg.selectAll('.link.target-' + d.id).each(function (d) {
    ...
    function outgoing(d, over) {
      if (data.highlightOutgoingEdges) {
        svg.selectAll('.link.source-' + d.id).each(function (d) {
          highlightEdge(d, over)
        })
      }
    }
    ...
    // vertices
    var nodes = svg.selectAll('g.node').data(data.nodes, function (d) {
      return d.id
    })
    var g = nodes
      .enter()
    ...
    g.append('circle')
      ...
      .each(function (d) {
        ...
        if (parent.classed('highlight')) {
          d3.select(this).attr('stroke-width', 3).attr('stroke', color.range()[1])
        }
      })
    g.append('text')
      .attr('fill', 'white')
    ...
    nodeExit.selectAll('circle').transition().attr('r', 0)
    ...
    function update(transition) {
      ...
      ;(transition ? paths.transition().ease('linear') : paths).attr('d', line)
      links.each(function (d) {
      ...
      weights.attr('transform', function (d) {
      ...
      ;(transition ? nodes.transition().ease('linear') : nodes).attr('transform', function (d) {
        return transform({
          translate: d
        })
      })
