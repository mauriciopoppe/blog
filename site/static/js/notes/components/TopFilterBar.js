...
export function TopFilterBar({
  ...
  return html`
    ...
    <header class="tw-absolute tw-top-3 sm:tw-top-4 tw-left-2 tw-right-2 sm:tw-left-4 sm:tw-right-4 tw-pointer-events-none tw-flex tw-items-center tw-justify-center tw-z-30">
      <div
        class="tw-pointer-events-auto tw-flex tw-items-center tw-gap-1 tw-p-1 tw-rounded-full tw-bg-[var(--grey-dark)] tw-backdrop-blur-md tw-border tw-border-[var(--ring-border)] tw-shadow-subtle tw-max-w-full tw-overflow-hidden"
      >
        <!-- Home Button -->
        <a
          href="/"
          class="tw-flex tw-items-center tw-gap-1.5 tw-px-2.5 tw-py-1.5 tw-rounded-full hover:tw-text-primary tw-text-xs tw-font-sans tw-transition tw-text-[var(--grey-light)] tw-shrink-0"
          title="Back to Home"
        >
          <span class="material-symbols-outlined" style="font-size: 16px">home</span>
          <span class="tw-font-medium tw-hidden sm:tw-inline">Home</span>
        </a>

        <!-- Vertical Separator -->
        <div class="tw-w-[1px] tw-h-4 tw-bg-white/15 tw-shrink-0"></div>

        <!-- Center: Category Filter Pills & Search in a single scrollable row -->
        <div
          id="graph-filters"
          class="tw-flex tw-flex-nowrap tw-items-center tw-gap-1 tw-overflow-x-auto tw-min-w-0"
        >
            ...
            ? html`
                ...
                >
                  ...
                  <input
                    ...
                    class="tw-bg-transparent tw-border-none tw-outline-none tw-text-xs tw-text-[var(--grey-lighter)] placeholder:tw-text-[var(--grey-light)]/50 tw-w-28 sm:tw-w-44 md:tw-w-56 tw-font-sans"
                  />
                  ${searchQuery
                    ? html`
                        <button
        ...
        <!-- Theme Toggle -->
        <div class="tw-flex tw-items-center tw-shrink-0 tw-px-0.5">
          <button
            onClick=${() => toggleTheme('dark')}
            class="theme-toggle tw-flex tw-h-7 tw-w-7 tw-items-center tw-justify-center tw-rounded-full hover:tw-text-primary tw-transition tw-hidden light:tw-flex"
            data-nexttheme="dark"
            title="Dark theme"
          >
            <span class="material-symbols-outlined light:tw-text-zinc-800" style="font-size: 16px">dark_mode</span>
          </button>
          <button
            onClick=${() => toggleTheme('light')}
            class="theme-toggle tw-flex tw-h-7 tw-w-7 tw-items-center tw-justify-center tw-rounded-full hover:tw-text-primary tw-transition light:tw-hidden"
            data-nexttheme="light"
            title="Light theme"
          >
            <span class="material-symbols-outlined dark:tw-text-zinc-200" style="font-size: 16px">light_mode</span>
          </button>
        </div>
      </div>
    </header>
